...
<div class="admin-content">
    ...
    <div class="admin-main-content main-container">
        <div class="container-fluid p-0">
            ...
            <h1 class="h3 mb-3"><strong>Edit Teacher</strong></h1>
                            ...
                            <form action="{{ route('teachers.update', $teacher->id) }}" method="POST">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="email" class="form-control @error('email_id') is-invalid @enderror"
                                           id="email_id" name="email_id" value="{{ old('email_id', $teacher->email_id) }}" required>
                                    @error('email_id')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control @error('mobile_no') is-invalid @enderror"
                                           id="mobile_no" name="mobile_no" value="{{ old('mobile_no', $teacher->mobile_no) }}" required>
                                    @error('mobile_no')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('status') is-invalid @enderror"
                                        ...
                                        <option value="pending" {{ old('status', $teacher->status) == 'pending' ? 'selected' : '' }}>Pending</option>
                                        <option value="active" {{ old('status', $teacher->status) == 'active' ? 'selected' : '' }}>Active</option>
                                        <option value="suspend" {{ old('status', $teacher->status) == 'suspend' ? 'selected' : '' }}>Suspend</option>
                                    </select>
                                    @error('status')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('institute_code') is-invalid @enderror" id="instituteName" name="institute_code">
                                        ...
                                        @foreach ($institutes as $institute)
                                            <option value="{{ $institute->institute_code }}"
                                                {{ old('institute_code', $teacher->institute_code) == $institute->institute_code ? 'selected' : '' }}>
